...
function doPost(e) {
  ...
  // --- NEW: Telegram webhook routing ---
  if (_isTelegramUpdate_(payload)) {
    try {
      handleTelegramUpdate(payload); // (định nghĩa ở bot.gs)
      logRow("INFO", "TG/UPDATE", "", "Handled Telegram update", { has_msg: !!payload.message, uid: payload.update_id });
      return out.setContent(JSON.stringify({ ok: true }));
    } catch (err2) {
      logRow("ERROR", "TG/UPDATE", "", "Handle failed", { error: String(err2) });
      return out.setContent(JSON.stringify({ ok: false, error: "telegram_handle_failed" }));
    }
  }

...
class HsbcCcNoticeParser {
  ...
  parse(m) {
    ...
    // Dấu cho HSBC: không dấu = chi; có '-' hoặc '+' (hiếm) coi như thu; có từ khoá refund => thu
    let sign = -1;
    const sNear = this._peekSignNearSpan_(body, g.span);
    if (sNear === -1 || sNear === +1) sign = +1;
  ...
  _extractVndAmountInText_(text) {
    if (!text) return null;
    // Cả hai định dạng: "VND397,250" hoặc "397,250 VND" (hỗ trợ NBSP)
  ...
  _kvLabelVndInt_(text, labelRegex) {
    // Trả số nguyên (không “VND”), để định dạng lại theo yêu cầu

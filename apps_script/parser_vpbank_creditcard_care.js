 ...
 * Quy tắc dấu: '-' = chi (âm), '+' = thu (dương). Nếu không thấy dấu → mặc định thu.
...
class VpbankCcCareParser {
  ...
  parse(m) {
    ...
    if (amountLine) {
      ...
      else sign = +1; // default income when no explicit sign
    } else {
      const g = findAmountGeneric_(b);
      if (!g) return null;
      abs = g.abs;
      const sNear = (g.signHint !== null && g.signHint !== undefined) ? g.signHint : this._peekSignNearSpan_(b, g.span);
      if (sNear === +1) sign = +1;
    ...
    const availVal = this._valueFromLabelLine_(b, ['Hạn mức còn lại', 'Available Limit']) ||
      nearValueForLabels_(b, ['Hạn mức còn lại', 'Available Limit'], (s) => /\bVND\b/i.test(s)) || '';
  ...
  _valueFromLabelLine_(body, labels) {
    const lines = String(body || '').split(/\r?\n/);
    const lowerLabels = labels.map(lb => String(lb).toLowerCase());
    for (const lineRaw of lines) {
      const line = String(lineRaw || '').trim();
      if (!line) continue;
      const l = line.toLowerCase();
      if (!lowerLabels.some(lb => l.includes(lb))) continue;
      let m = line.match(/([0-9][0-9\.,\u00A0\u202F ]+)\s*VND/i);
      if (m && m[1]) return m[1].trim().replace(/\u00A0|\u202F/g, ' ') + ' VND';
      m = line.match(/VND\s*([0-9][0-9\.,\u00A0\u202F ]+)/i);
      if (m && m[1]) return m[1].trim().replace(/\u00A0|\u202F/g, ' ') + ' VND';
    }
    return '';
  }
